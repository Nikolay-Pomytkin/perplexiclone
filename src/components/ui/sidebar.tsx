...
const SidebarProvider = React.forwardRef<HTMLDivElement, SidebarProviderProps>(
  ({ defaultOpen = true, open: openProp, onOpenChange: setOpenProp, className, style, children, ...props }, ref) => {
    ...
    // This is a simple implementation, you can use a more advanced one if needed
    ...
    return (
      <SidebarContext.Provider value={contextValue}>
        <div
          ref={ref}
          className={cn("group/sidebar-wrapper flex h-screen w-full has-data-[variant=inset]:bg-sidebar overflow-hidden", className)}
          style={
            {
  ...
  if (isMobile) {
    return (
      <div
        ...
        className={cn(
          "group peer fixed inset-y-0 z-50 h-svh w-[--sidebar-width] bg-sidebar text-sidebar-foreground transition-transform duration-200 ease-linear data-[state=collapsed]:-translate-x-full data-[side=left]:left-0 data-[side=right]:right-0",
          className
        )}
        {...props}
      >
  ...
  return (
    <div
      ...
      data-variant={variant}
        data-side={side}
        className={cn(
          "group peer hidden md:flex text-sidebar-foreground transition-[width] duration-200 ease-linear h-full",
        "data-[collapsible=icon]:data-[state=collapsed]:w-[--sidebar-width(self)]",
    ...
    >
      <div className={cn(
        "flex h-full w-[--sidebar-width] flex-col bg-sidebar group-data-[collapsible=icon]:group-data-[state=collapsed]:!w-[--sidebar-width-icon] overflow-hidden",
        variant === "floating" ? "rounded-lg m-2" : "",
    ...
    return (
      <button
        ...
        className={cn(
          ...
          "group-data-[collapsible=icon]:opacity-0 group-data-[collapsible=icon]:delay-300",
          className
        )}
        {...props}
      />
...
>(({ className, ...props }, ref) => {
  return <Separator ref={ref} className={cn("bg-sidebar-border", className)} {...props} />
})
...
const SidebarMenuButton = React.forwardRef<
  ...
  React.ComponentProps<"button"> & {
    asChild?: boolean
    isActive?: boolean
    tooltip?: string | React.ComponentPropsWithoutRef<typeof React.Fragment>
